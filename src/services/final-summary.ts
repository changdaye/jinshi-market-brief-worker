...
async function summarizeWithLLM(config: BriefConfig, ai: Ai | undefined, messages: string[]): Promise<{ content: string; modelLabel: string }> {
  const fallback = [
    "【凌晨总结】",
    `过去 ${messages.length} 条飞书消息已归档。`,
    "【简述】",
    messages.slice(0, 3).map((message, index) => `${index + 1}. ${message.split("\n")[0] ?? message}`).join("\n"),
  ].join("\n\n");
    ...
    try {
      ...
      if (response.ok) {
        ...
        const content = typeof raw === "string"
          ? raw.trim()
          : raw?.map((part: any) => part.text ?? "").join("").trim();
  ...
  try {
    const result = await ai.run(config.llmModel.startsWith("@cf/") ? config.llmModel : "@cf/meta/llama-3.2-1b-instruct", {
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: payload },
      ],
    ...
    return content
      ? { content, modelLabel: config.llmModel.startsWith("@cf/") ? config.llmModel : "Llama 3.2 1B Instruct" }
      : { content: fallback, modelLabel: "" };
  } catch (error) {
...
export async function runFinalSummary(env: Env, config: BriefConfig, now = new Date()): Promise<FinalSummaryResult | null> {
  const prefix = "jinshi-market-brief-worker/feishu-messages/";
  ...
  const recent = objects
    .map((item) => ({ ...item, timestamp: parseTimestampFromKey(item.key) }))
    .filter((item): item is typeof item & { timestamp: number } => item.timestamp != null && item.timestamp >= cutoff)
